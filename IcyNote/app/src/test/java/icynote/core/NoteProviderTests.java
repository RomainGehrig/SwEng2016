...
public abstract class NoteProviderTests {
    ...
    protected Note createNoteAndCheck() {
        Optional<Note> n = toTest.createNote();
        assertNotNull(n);
    ...
    public void getNonExistingNoteTest() {
        Optional<Note> n = toTest.getNote(0);
        assertNotNull(n);
    ...
    public void persistWithTitleModification() {
        ...
        Optional<Note> m = toTest.getNote(n.getId());
    ...
    public void deleteAnExistingNote() {
        ...
        Optional<Note> note = toTest.getNote(validId);
    ...
    public void deleteDoesNotAffectOtherNotes() {
        ...
        int notesSizeBeforeDelete = size(toTest.getNotes(OrderBy.TITLE, OrderType.ASC));

        Response response = toTest.delete(invalidID);
        assertFalse(response.isPositive());

        int notesSizeAfterDelete = size(toTest.getNotes(OrderBy.TITLE, OrderType.ASC));

        assertEquals(notesSizeBeforeDelete, notesSizeAfterDelete);
    }
    ...
    public void getNotesDefensiveCopy() {
        ...
        Iterable<Note> notes2 = toTest.getNotes(OrderBy.TITLE, OrderType.ASC);
    ...
    public void getNotesByTitleAscTest() {
        getNotes(OrderBy.TITLE, new FieldExtractor() {
            @Override
            public Comparable getField(Note n) {
                return n.getTitle();
            }
        }, true);
    }
    ...
    private void getNotes(OrderBy by, final FieldExtractor get, final boolean asc) {
        ...
        GregorianCalendar d = new GregorianCalendar(2000, 1, 1);
        ...
        Iterable<Note> notes = toTest.getNotes(by, tpe);
        assertNotNull("note iterable should not be null", notes);

        checkOrder(notes.iterator(), new OrderPredicate<Note>() {
            @Override
            public boolean ordered(Note lhs, Note rhs) {

                int comp = get.getField(lhs).compareTo(get.getField(rhs));
                boolean res = (asc) ? comp <= 0 : comp >= 0;
                if (!res) {
                    System.out.println(get.getField(lhs).toString());
                    System.out.println(get.getField(rhs).toString());
                }
                return res;
            }
        });
    ...
    private int size(Iterable<Note> collection) {
        int sz = 0;
    ...
    private abstract class FieldExtractor {
        public abstract Comparable getField(Note n);
    }

    private abstract class OrderPredicate<T> {
        public abstract boolean ordered(T lhs, T rhs);
    }
}
