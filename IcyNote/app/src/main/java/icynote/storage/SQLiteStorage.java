...
public class SQLiteStorage implements Storage {
    private static final String LOGGING_TAG = "SQLiteStorage";
    ...
    public SQLiteStorage(Context context, String userUID) {
    ...
    public Optional<Note> createNote() {
        ...
        Note note = createNote(noteId, now);
    ...
    @NonNull
    private static Note getNoteFromCursor(@NonNull Cursor cursor) {
        Note note = createNote(getNoteId(cursor), null);
    ...
    @NonNull
    private static Note createNote(long id, GregorianCalendar now) {
        NoteData data = new NoteData();
        data.setId((int) id);
        data.setCreation(now);
        data.setLastUpdate(now);
        return new ConstIdInteractor(data);
    }
