...
/**
 * Stacks {@code NoteInteractor}(s) on top of the {@code Note} object provided by the
 * storage. The class also performs input validation by testing for {@code null} values.
 *
 * @author Julien Harbulot
 * @version 1.0
 * @see icynote.coreImpl.noteInteractors.NoteInteractor
 */
public class IcyNoteCore implements icynote.core.IcyNoteCore {
    private final CheckedStorage storage;
    private NoteInteractorFactory wrapper = new NoteInteractorFactory();

    //--------------------------------------------------------------

    public IcyNoteCore(Storage s) {
    ...
    public void stack(NoteInteractorFactory interactorFactory) {
        Checker.checkNotNull(interactorFactory);
    ...
    public Iterable<Note> getNotes(OrderBy index, OrderType order) {
        Checker.checkNotNull(index);
        Checker.checkNotNull(order);
